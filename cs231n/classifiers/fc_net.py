...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    drelu1, dw2, db2 = affine_backward(dsoftmax, scores_cache)
    grads['W2'] = dw2 + self.reg * self.params['W2']
    grads['b2'] = db2

    daffine1 = relu_backward(drelu1, relu1_cache)
    dx, dw1, db1 = affine_backward(daffine1, affine1_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i, dim in enumerate(hidden_dims):
      #Hidden Weights
      self.params['W' + str(i + 1)] = np.random.normal(0, weight_scale, (past_dim, dim))
      #Hidden Biases
      self.params['b' + str(i + 1)] = np.zeros((1, dim))
  ...
  def loss(self, X, y=None):
    ...
    scores, scores_cache = affine_forward(next_input, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    #Softmax grad
    d_softmax_input, dw_softmax, db_softmax = affine_backward(dsoftmax, scores_cache)
    grads['W' + str(self.num_layers)] = dw_softmax + self.reg * self.params['W' + str(self.num_layers)]
    grads['b' + str(self.num_layers)] = db_softmax
